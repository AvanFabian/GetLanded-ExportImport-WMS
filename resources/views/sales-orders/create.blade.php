...
@section('content')
   <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Header -->
      <div class="mb-6">
         <h1 class="text-3xl font-bold text-gray-900">{{ __('app.add_new_sales_order') }}</h1>
         <p class="mt-1 text-sm text-gray-600">{{ __('app.add_new_sales_order') }}</p>
      </div>
      ...
      <!-- Form -->
      <form action="{{ route('sales-orders.store') }}" method="POST" id="salesOrderForm" class="space-y-6">
         @csrf
         ...
         <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="mt-6">
               ...
               <textarea id="notes" name="notes" rows="3"
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="{{ __('app.order_notes') }}">{{ old('notes') }}</textarea>
            </div>
         </div>
                  ...
                  <thead class="bg-gray-50">
                     <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                           {{ __('app.product') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('app.qty') }}
                        </th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">
            ...
            <div class="max-w-md ml-auto space-y-3">
               ...
               <div class="flex justify-between text-sm items-center">
                  ...
                  <div class="flex items-center gap-2">
                     ...
                     <input type="number" id="discount" name="discount" value="{{ old('discount', 0) }}" min="0"
                        step="1"
      ...
      function addItem() {
        ...
        <td class="px-4 py-3">
            <select name="items[${itemIndex}][product_id]" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required onchange="updatePrice(${itemIndex})">
                ...
                ${products.map(p => `<option value="${p.id}" data-price="${p.selling_price || 0}">${p.name} - ${p.category?.name || ''}</option>`).join('')}
            </select>
        </td>
        <td class="px-4 py-3">
        ...
        <td class="px-4 py-3">
            <input type="number" name="items[${itemIndex}][unit_price]" min="0" step="0.01" value="0" required
