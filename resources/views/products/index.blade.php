...
@section('content')
   ...
   <form id="printLabelsForm" action="{{ route('products.print-labels') }}" method="POST" style="display: none;">
    ...
    <!-- Import Modal -->
    <div x-data="{ showImportModal: false }" @keydown.escape.window="showImportModal = false">
        <!-- Trigger Button (Replaces the old Link) -->
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const importBtnContainer = document.querySelector('.flex.gap-2');
                if(importBtnContainer) {
                    const btn = document.createElement('button');
                    btn.className = 'px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600';
                    btn.innerHTML = `<svg class="w-5 h-5 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg> Import`;
                    btn.onclick = () => { document.querySelector('[x-data]').__x.$data.showImportModal = true; };
                    
                    // Replace the old link or append
                    // Finding the old link to replace might be tricky via JS strictly, 
                    // easier to just edit the blade template directly. 
                    // BUT I am in `replace_file_content` of the Bottom Script section.
                    // Accessing x-data from outside is hacky.
                    // Better approach: I will edit the BUTTON definition in the TOP of the file in the next step.
                    // For now, I'll just place the Modal HTML here.
                }
            });
        </script>
